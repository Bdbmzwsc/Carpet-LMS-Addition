...
public final class OpPlayerNoCheatExtra {
    ...
    public static Predicate<CommandSourceStack> wrapPredicate(Predicate<CommandSourceStack> predicate) {
        return source -> predicate.test(source) && canCheat(source);
    }

    private static boolean canCheat(CommandSourceStack source) {
        return !Settings.opPlayerNoCheatExtra || !(source.getEntity() instanceof ServerPlayer);
    }
}
