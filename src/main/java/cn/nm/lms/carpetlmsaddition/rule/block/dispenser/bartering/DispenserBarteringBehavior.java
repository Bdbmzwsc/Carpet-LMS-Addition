...
public class DispenserBarteringBehavior extends DefaultDispenseItemBehavior {
    ...
    DispenserBarteringBehavior(BooleanSupplier isBarteringEnabled, DispenseItemBehavior fallbackDispenseBehavior) {
    ...
    protected @NonNull ItemStack execute(@NonNull BlockSource source, @NonNull ItemStack stack) {
        ...
        DispenserBarterInputParser.ParseResult parseResult =
            DispenserBarterInputParser.parseBarterInput(stack.copyWithCount(1));
        if (!parseResult.valid() || parseResult.barterRollCount() <= 0) {
            return stack;
        }

        stack.shrink(1);
        for (ItemStack shulkerBox : parseResult.shulkerBoxesToDrop()) {
    ...
    private @NonNull ItemStack runFallbackDispense(@NonNull BlockSource source, @NonNull ItemStack stack) {
