...
public final class CommandLms implements BaseCommand {
    ...
    private static boolean cannotUse(CommandSourceStack src, ServerPlayer target) {
        ...
        String perm = isSelf ? Settings.commandLMSSelf : Settings.commandLMSOthers;
        return !CommandHelper.canUseCommand(src, perm);
    }

    private static ServerPlayer getTarget(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
    ...
    private static CompletableFuture<Suggestions> suggestConfigs(CommandContext<CommandSourceStack> _context,
        SuggestionsBuilder builder) {
        configList().forEach(builder::suggest);
        return builder.buildFuture();
    }

    private static CompletableFuture<Suggestions> suggestValues(CommandContext<CommandSourceStack> ctx,
        SuggestionsBuilder builder) {
