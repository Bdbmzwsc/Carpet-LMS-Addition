...
final class DispenserBarteringExecutor {
    private static final ResourceKey<LootTable> PIGLIN_BARTER_TABLE = ResourceKey.create(Registries.LOOT_TABLE,
    ...
    static void dropBarterLoot(BlockSource source, int barterRollCount) {
        ...
        LootTable table = level.getServer().reloadableRegistries().getLootTable(PIGLIN_BARTER_TABLE);
        LootParams params = new LootParams.Builder(level).create(LootContextParamSets.EMPTY);
        List<AccumulatedDrop> accumulatedDrops = new ArrayList<>();

        for (int i = 0; i < barterRollCount; i++) {
        ...
        for (AccumulatedDrop accumulatedDrop : accumulatedDrops) {
    ...
    static boolean shouldBlockByDispenserName(BlockSource source) {
        ...
        return !CheckName.checkName(name, Settings.dispenserBarteringName, "bartering");
    }
